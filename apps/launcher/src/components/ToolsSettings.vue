...
<template>
  ...
  <p>Choose the features you use. Settings and shortcuts apply to every account.</p>
    <div class="setting-group tools-master">
      ...
      <div v-if="snapshot.tools.restartRequired" class="restart-row"><span><strong>Restart the launcher</strong><small>Your selection is saved. Tools load when the launcher starts. Restarting only a game window will not apply this change.</small><small v-if="hasActiveGames">Close every game window before restarting the launcher.</small></span><button class="primary" :disabled="hasActiveGames" @click="perform(() => api?.tools.restartToApply())">Restart launcher</button></div>
