import { describe, expect, it } from "vitest";
import { mount } from "@vue/test-utils";
import SkillDetails from "./SkillDetails.vue";
import { ELITE_FIXTURE_SKILLS } from "../elite-fixture";

describe("shared skill details", () => {
  it("shows costs in the game's order and notation", () => {
    const wrapper = mount(SkillDetails, { props: { skill: { ...ELITE_FIXTURE_SKILLS[0]!,
      energyCost: 5, adrenalineCost: 8, activationSeconds: 1.5, rechargeSeconds: 12,
      healthCost: 10, overcast: 5, aftercastSeconds: 0.75 } } });
    expect(wrapper.findAll('.skill-stats img').map(icon => icon.attributes('alt'))).toEqual([
      'Adrenaline', 'Energy', 'Overcast', 'Health sacrifice', 'Activation', 'Recharge',
    ]);
    expect(wrapper.findAll('.skill-stats dd').map(value => value.text())).toEqual(['8', '5', '5', '10%', '1½', '12']);
    expect(wrapper.get('.skill-stats [title^="Activation"]').attributes('title')).toBe('Activation: 1.5 seconds');
    expect(wrapper.find('.skill-aftercast').exists()).toBe(false);
    expect(wrapper.get('.skill-type').text()).toBe('Elite Axe Attack');
    expect(wrapper.get('.skill-scaled').text()).toBe('10...45');
    expect(wrapper.findAll('.skill-stats img').every(icon => icon.attributes('src')?.includes('.png'))).toBe(true);
    expect(wrapper.text()).toContain('Warrior');
    wrapper.unmount();
  });
  it("notes only unusual aftercast and writes quarter seconds as fractions", () => {
    const wrapper = mount(SkillDetails, { props: { skill: { ...ELITE_FIXTURE_SKILLS[5]!,
      activationSeconds: 0.25, aftercastSeconds: 0, rechargeSeconds: 0 } } });
    expect(wrapper.findAll('.skill-stats dd').map(value => value.text())).toEqual(['5', '¼', '0']);
    expect(wrapper.get('.skill-type').text()).toBe('Elite Spell');
    wrapper.unmount();
  });
  it("omits zero mechanics and explains unavailable descriptions without inventing values", () => {
    const wrapper = mount(SkillDetails, { props: { skill: { ...ELITE_FIXTURE_SKILLS[0]!,
      energyCost: 0, adrenalineCost: 0, activationSeconds: 0, rechargeSeconds: 0,
      healthCost: 0, overcast: 0, aftercastSeconds: 0, description: '' } } });
    expect(wrapper.find('.skill-stats').exists()).toBe(false);
    expect(wrapper.text()).toContain('Description is unavailable from this installed client.');
    wrapper.unmount();
  });
});
