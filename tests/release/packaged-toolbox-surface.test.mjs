...
// Not an artifact assertion, and not executed anywhere: it reads
// src/main/client-runtime.ts for the composition that keeps template saving
// working when the Toolbox transform cannot run. P4.5 replaced this file's
// artifact and gate regexes; this claim has no artifact form, and deleting it
// would leave the invariant unproved, so it stays until client-runtime gets an
// executed test of its own.
test("an uncertified client build still serves the template-save client", () => {
  const runtime = readFileSync(path.join(root, "src/main/client-runtime.ts"), "utf8");
