...
import {
  TOOLBOX_BUILDS,
  toolboxLayoutWords,
} from "../src/main/core/toolbox-builds.ts";
import {
  TOOLBOX_CURSOR_ABI,
...
const TOOLBOX_BUILD =
  TOOLBOX_BUILDS[0] ?? assert.fail("the canonical Toolbox build table is empty");
...
function toolboxResource(url: string) {
...
async function driveHarnessRuntime(page: Page) {
  ...
  return page.evaluate(() =>
    new Promise<{ manifestSections: number; toolboxHookExports: string[] }>(
      (resolve, reject) => {
...
async function assertPackagedOffSession() {
  ...
  try {
    ...
    assert.deepEqual(
      await fixture.page.evaluate(() => ({
        installations: window.gwToolboxInstallations,
        runtime: window.gwToolboxRuntime,
        state: window.gwToolboxState,
      })),
      {
...
async function installTargetReadout(page: Page, moduleBytes: Uint8Array) {
  return page.evaluate(async (bytes: number[]) => {
    ...
    // build of `src/renderer/toolbox.ts`.
    const specifier = "./toolbox.js";
    const { installToolbox }: typeof import("../src/renderer/toolbox.ts") =
...
async function assertTargetReadoutLifecycle() {
  ...
  try {
    ...
    assert.deepEqual(
      await fixture.page.evaluate(() => window.gwNative.init.toolboxSelection),
      { nativeCursor: false, targetReadout: true },
    );
    ...
    await fixture.page.waitForFunction(() => {
      // `gwToolboxRuntime` is declared as an open record, so the rendered line
