...
const KNOWN_UNCHECKED = [
  {
    reason:
      "Deferred to Phase 4 of plans/typescript.md. These convert to .ts and are " +
      "annotated once there; enrolling them as .mjs now would mean writing JSDoc " +
      "types that the conversion immediately deletes.",
    files: [
      "tests/electron/a-launch-reaches-github-only-when-asked.spec.mjs",
      "tests/electron/app.spec.mjs",
      "tests/electron/client-compatibility.spec.mjs",
      "tests/electron/client-runtime-concurrency.spec.mjs",
      "tests/electron/diagnostics.spec.mjs",
      "tests/electron/fixtures.mjs",
      "tests/electron/input.spec.mjs",
      "tests/electron/launcher.spec.mjs",
      "tests/electron/live.spec.mjs",
      "tests/electron/playwright.config.mjs",
      "tests/electron/sandbox.spec.mjs",
      "tests/electron/settings.spec.mjs",
      "tests/electron/toolbox-cursor.spec.mjs",
      "tests/electron/toolbox-runtime.spec.mjs",
      "tests/fixtures/kill-mid-atomic-write.mjs",
      "tests/helpers/package-inventory.mjs",
      "tests/integration/sockets.test.mjs",
      "tests/integration/toolbox.test.mjs",
      "tests/integration/updater.test.mjs",
      "tests/packaged-smoke.mjs",
      "tests/packaged-toolbox-runtime.mjs",
      "tests/policy/action-pinning.test.mjs",
      "tests/policy/font-license.test.mjs",
      "tests/policy/forbidden-artifacts.test.mjs",
      "tests/policy/fuses.test.mjs",
      "tests/policy/import-boundaries.test.mjs",
      "tests/policy/lint-coverage.test.mjs",
      "tests/policy/markdown-links.test.mjs",
      "tests/policy/package-application-icon.test.mjs",
      "tests/policy/source-main-process-security-guards.test.mjs",
      "tests/policy/source-preload-body-holds-no-channel-copy.test.mjs",
      "tests/policy/source-release-pipeline.test.mjs",
      "tests/policy/source-saved-login-surface.test.mjs",
      "tests/policy/source-wasm-host.test.mjs",
      "tests/policy/the-benchmark-measures-each-arm-in-both-orders.test.mjs",
      "tests/policy/typecheck-coverage.test.mjs",
      "tests/release/packaged-toolbox-surface.test.mjs",
      "tests/release/preload-behaviour.test.mjs",
      "tests/release/source-agrees-with-the-shared-contracts.test.mjs",
      "tests/website-smoke.mjs",
    ],
  },
  {
    reason:
      "These install the TypeScript loader every other test runs under, so they " +
      "cannot themselves be resolved through it. They stay JavaScript; Phase 4 " +
      "enrols them by widening the include glob rather than by converting them.",
    files: ["tests/ts-hook.mjs", "tests/ts-resolve.mjs"],
  },
];
...
const isChecked = (file) => checked.has(path.join(root, file));
