...
// package-application-icon.test.mjs), the credentials surface
// (source-saved-login-surface.test.mjs), the security posture (executed against
// a real window in tests/electron/sandbox.spec.mjs), and the three assertions
...
test("packaging cleans its output first, and builds the renderer program", () => {
  ...
  assert.match(scripts.make, /scripts\/clean-output\.mjs/);
  assert.match(scripts.package, /scripts\/clean-output\.mjs/);
...
test("the website suite runs on its own path-filtered workflow", () => {
  ...
  assert.doesNotMatch(json("package.json").scripts.verify, /test:website/);
});
