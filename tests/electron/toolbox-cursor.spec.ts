...
type CursorConsumerState = ReturnType<
  typeof import("../../src/renderer/toolbox-cursor.js").createCursorConsumer
>["state"];
...
// The kernel side is covered by tests/integration/toolbox.test.ts; this is
...
async function driveCursor(
  ...
  return page.evaluate(async (script) => {
    ...
    const { createCursorConsumer } = await importRenderer<
      typeof import("../../src/renderer/toolbox-cursor.js")
    >(new URL("toolbox-cursor.js", globalThis.location.href).href);
    const BYTES = 4160;
    ...
    try {
      expect(
        await optedOut.page.evaluate(
          () => window.gwNative.init.toolboxSelection.nativeCursor,
        ),
      ).toBe(false);
    } finally {
