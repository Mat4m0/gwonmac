...
test.describe("renderer pointer input", () => {
  test("releases held keys and buttons on an input reset", async () => {
    const fixture = await launchPlayableClient("gw-input-e2e-");
    try {
      const { page } = fixture;
  ...
  test("passes every click run and primary drag through without touch input", async () => {
    const fixture = await launchPlayableClient("gw-macos-pointer-e2e-");
    try {
      const { page } = fixture;
      await startGameInput(page);
  ...
  test("releases only pointer input when the pointer leaves the app window", async () => {
    const fixture = await launchPlayableClient("gw-input-window-leave-e2e-");
    try {
      const { page } = fixture;
      await startGameInput(page);
      await page.evaluate(() => {
  ...
  test("keeps the game canvas interactive through every renderer edge", async () => {
    const fixture = await launchPlayableClient("gw-input-viewport-edges-e2e-");
    try {
      const { page } = fixture;
      await startGameInput(page);
  ...
  test("accumulates trackpad pixels without changing discrete wheel input", async () => {
    const fixture = await launchPlayableClient("gw-wheel-e2e-");
    try {
