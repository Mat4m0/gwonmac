...
import { PatchClient } from "../../src/main/core/patch-client.ts";
import type { PatchFetch } from "../../src/main/core/patch-transport.ts";
import { createBoundedPatchFetch } from "../../src/main/core/patch-transport.ts";
import {
...
import type {
  ManifestFileEntry,
  RawManifest,
} from "../../src/main/core/manifest.ts";
import { isProxyRoute, resolveProxyHost } from "../../src/main/core/proxy-routes.ts";
import { parseRangeHeader } from "../../src/main/core/ranges.ts";
import { HttpStatusError } from "../../src/shared/errors.ts";
import type { DownloadProgress } from "../../src/shared/contracts.ts";
...
describe("integration: patch updater", () => {
  ...
  it("publishes JSPI artifacts from a local fixture and skips unchanged", async () => {
    ...
    const fetchFixture: PatchFetch = async (url) => {
      const pathname = new URL(url).pathname;
      if (pathname === "/manifest.json") {
        return { status: 200, body: new TextEncoder().encode(JSON.stringify(manifest)) };
      }
      const hash = /^\/([0-9a-f]+)\.bin$/.exec(pathname)?.[1];
      const body = hash === undefined ? undefined : store.get(hash);
      return body
    ...
    const phases: DownloadProgress["phase"][] = [];
    ...
    const jsHash = manifestFiles.find((file) => file.name === "Gw.jspi.js")
      ?.chunkHashes[0];
    assert(jsHash);
    const jsChunk = store.get(jsHash);
    assert(jsChunk);
    await writeFile(join(artifacts, "Gw.jspi.js"), Buffer.alloc(js.length, 0x59));
    await writeFile(join(chunks, jsHash), Buffer.alloc(jsChunk.length, 0x5a));
    ...
    await assert.rejects(
      () => interrupted.update(),
      (error) => error instanceof HttpStatusError && error.status === 404,
    );
    assert.equal((await readFile(join(artifacts, "Gw.jspi.js"))).toString(), js.toString());
