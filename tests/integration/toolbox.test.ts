...
} from "../../src/renderer/toolbox-snapshot.ts";
...
type SnapshotRead = ReturnType<typeof readToolboxSnapshot>;
type CursorHeaderRead = ReturnType<typeof readToolboxCursorHeader>;
type CursorPixelsRead = ReturnType<typeof readToolboxCursorPixels>;
 ...
 * callable; the arities come from `src/toolbox-kernel/lib.rs`, and every
...
function kernelExports(exports: WebAssembly.Exports) {
  const init = exports["toolbox_init"];
  const tick = exports["toolbox_tick"];
  if (typeof init !== "function" || typeof tick !== "function") {
    throw new Error("toolbox-kernel.wasm did not export toolbox_init/toolbox_tick");
  }
...
describe("Toolbox snapshot ABI", () => {
  it("decodes stable player and target state", () => {
    const state = decoded(readToolboxSnapshot(snapshot(), 0));
  ...
  it("rejects torn, incompatible, loading, and absent-target snapshots", () => {
    assert.equal(
      rejected(readToolboxSnapshot(snapshot({ sequence: 3 }), 0)),
      "writing",
    );
    const corrupt = snapshot();
    new DataView(corrupt).setUint16(4, 2, true);
    assert.equal(rejected(readToolboxSnapshot(corrupt, 0)), "snapshot");
    assert.equal(
      rejected(readToolboxSnapshot(snapshot({ flags: 8 }), 0)),
      "loading",
    );
    const noTarget = decoded(readToolboxSnapshot(snapshot({ flags: 3 }), 0));
  ...
  it("rejects unknown flags, invalid identities, bands, and non-finite values", () => {
    assert.equal(
      rejected(readToolboxSnapshot(snapshot({ flags: 0x10 }), 0)),
      "snapshot",
    );
    assert.equal(
      rejected(readToolboxSnapshot(snapshot({ playerId: 0 }), 0)),
      "corrupt",
    );
    assert.equal(
      rejected(readToolboxSnapshot(snapshot({ rangeBand: 9 }), 0)),
      "corrupt",
    );
    assert.equal(
      rejected(readToolboxSnapshot(snapshot({ distance: Number.NaN }), 0)),
      "corrupt",
    );
  });
});
...
describe("Toolbox cursor region ABI", () => {
  it("decodes a published cursor and its RGBA payload", () => {
    ...
    const header = readyCursor(readToolboxCursorHeader(region, 0));
    ...
    const full = publishedPixels(readToolboxCursorPixels(region, 0));
  ...
  it("reports hidden and non-cursor states without inventing geometry", () => {
    ...
    const cleared = invalidCursor(
      readToolboxCursorHeader(cursorRegion({ flags: 0 }), 0),
    );
    ...
    const unsupported = invalidCursor(
      readToolboxCursorHeader(cursorRegion({ flags: CURSOR_UNSUPPORTED }), 0),
    );
  ...
  it("rejects a torn, foreign, or malformed cursor region", () => {
    const reason = (overrides: CursorOverrides) =>
      cursorReason(readToolboxCursorHeader(cursorRegion(overrides), 0));
    ...
    assert.equal(
      cursorReason(readToolboxCursorHeader(new ArrayBuffer(64), 0)),
      "memory",
    );
    assert.equal(cursorReason(readToolboxCursorHeader(cursorRegion(), 4)), "memory");
...
describe("Toolbox companion kernel", () => {
  it("calls the original once and publishes a checked snapshot", async () => {
    ...
    const state = decoded(
      readToolboxSnapshot(kernel.memory.buffer, ADDRESSES.snapshot),
    );
    assert.equal(state.status, "ready");
    ...
    for (const [distance, band] of boundaries) {
      ...
      kernel.tick();
      assert.equal(
        decoded(readToolboxSnapshot(kernel.memory.buffer, ADDRESSES.snapshot))
          .rangeBand,
        band,
      );
    }
    ...
    kernel.tick();
    assert.equal(
      rejected(readToolboxSnapshot(kernel.memory.buffer, ADDRESSES.snapshot)),
      "game",
    );
  ...
  it("collects only the explicitly enabled tools", async () => {
    ...
    readoutOnly.tick();
    const state = decoded(
      readToolboxSnapshot(readoutOnly.memory.buffer, ADDRESSES.snapshot),
    );
    assert.equal(state.status, "ready");
    assert.equal(state.tickCount, 1);
    assert.equal(readoutOnly.field(CURSOR.magic), 0);
