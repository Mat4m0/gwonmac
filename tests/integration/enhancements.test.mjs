...
async function createKernel() {
  ...
  return {
    ...
    init: (overrides = {}) => {
      ...
      return instance.exports.companion_init(
    ...
    tick: () => instance.exports.companion_tick(123),
...
describe("Companion snapshot ABI", () => {
  it("decodes stable player and target state", () => {
    const state = readCompanionSnapshot(snapshot(), 0);
  ...
  it("rejects torn, incompatible, loading, and absent-target snapshots", () => {
    assert.equal(
      readCompanionSnapshot(snapshot({ sequence: 3 }), 0).reason,
      "writing",
    );
    const corrupt = snapshot();
    new DataView(corrupt).setUint16(4, 2, true);
    assert.equal(readCompanionSnapshot(corrupt, 0).reason, "snapshot");
    assert.equal(
      readCompanionSnapshot(snapshot({ flags: 8 }), 0).reason,
      "loading",
    );
    const noTarget = readCompanionSnapshot(snapshot({ flags: 3 }), 0);
  ...
  it("rejects unknown flags, invalid identities, bands, and non-finite values", () => {
    assert.equal(
      readCompanionSnapshot(snapshot({ flags: 0x10 }), 0).reason,
      "snapshot",
    );
    assert.equal(
      readCompanionSnapshot(snapshot({ playerId: 0 }), 0).reason,
      "corrupt",
    );
    assert.equal(
      readCompanionSnapshot(snapshot({ rangeBand: 9 }), 0).reason,
      "corrupt",
    );
    assert.equal(
      readCompanionSnapshot(snapshot({ distance: Number.NaN }), 0).reason,
      "corrupt",
    );
  });
});
...
describe("Companion cursor region ABI", () => {
  it("decodes a published cursor and its RGBA payload", () => {
    ...
    const header = readCompanionCursorHeader(region, 0);
    ...
    const full = readCompanionCursorPixels(region, 0);
  ...
  it("reports hidden and non-cursor states without inventing geometry", () => {
    const hidden = readCompanionCursorHeader(
    ...
    const cleared = readCompanionCursorHeader(cursorRegion({ flags: 0 }), 0);
    ...
    const unsupported = readCompanionCursorHeader(
  ...
  it("rejects a torn, foreign, or malformed cursor region", () => {
    const reason = (overrides) =>
      readCompanionCursorHeader(cursorRegion(overrides), 0).reason;
    ...
    assert.equal(readCompanionCursorHeader(new ArrayBuffer(64), 0).reason, "memory");
    assert.equal(readCompanionCursorHeader(cursorRegion(), 4).reason, "memory");
...
describe("Companion kernel", () => {
  it("calls the original once and publishes a checked snapshot", async () => {
    ...
    const state = readCompanionSnapshot(kernel.memory.buffer, ADDRESSES.snapshot);
    assert.equal(state.status, "ready");
    ...
    for (const [distance, band] of boundaries) {
      ...
      kernel.tick();
      assert.equal(
        readCompanionSnapshot(kernel.memory.buffer, ADDRESSES.snapshot).rangeBand,
        band,
      );
    }
    ...
    kernel.tick();
    assert.equal(
      readCompanionSnapshot(kernel.memory.buffer, ADDRESSES.snapshot).targetValid,
      false,
    );
    ...
    kernel.tick();
    assert.equal(
      readCompanionSnapshot(kernel.memory.buffer, ADDRESSES.snapshot).targetId,
      9,
    );
    ...
    kernel.tick();
    assert.equal(
      readCompanionSnapshot(kernel.memory.buffer, ADDRESSES.snapshot).reason,
      "game",
    );
