...
describe("local client verification boundary", () => {
  ...
  it("rejects a relocation beyond the one bounded supported class", () => {
    const delta = 0x20_0000;
    const shifted = {
      ...ENHANCEMENT.layout,
      contextRoot: ENHANCEMENT.layout.contextRoot + delta,
      agentArray: ENHANCEMENT.layout.agentArray + delta,
      manualTargetAgentId: ENHANCEMENT.layout.manualTargetAgentId + delta,
      automaticTargetAgentId:
        ENHANCEMENT.layout.automaticTargetAgentId + delta,
      cursorActiveArt: ENHANCEMENT.layout.cursorActiveArt + delta,
      cursorSoftwareModel: ENHANCEMENT.layout.cursorSoftwareModel + delta,
      cursorShowCount: ENHANCEMENT.layout.cursorShowCount + delta,
      cursorColorBuffer: ENHANCEMENT.layout.cursorColorBuffer + delta,
    };
    assert.equal(isLocalClientVerification({
      ...valid(),
      enhancementBuild: { ...ENHANCEMENT, layout: shifted },
    }, TEMPLATE.sha256), false);
  });
