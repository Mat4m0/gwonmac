...
} from "../build/main/core/enhancement-builds.js";
import {
  COMPANION_CURSOR_ABI,
  COMPANION_CURSOR_BYTES,
  COMPANION_SNAPSHOT_ABI,
  COMPANION_SNAPSHOT_BYTES,
} from "../build/renderer/companion-snapshot.js";
...
const OFFICIAL_SHA256 = createHash("sha256")
  ...
  .digest("hex");
const ENHANCEMENT_BUILD = ENHANCEMENT_BUILDS[0];
assert.ok(ENHANCEMENT_BUILD, "the canonical Enhancement build table is empty");
...
function enhancementResource(url) {
...
async function assertPackagedOffSession() {
  ...
  try {
    ...
    assert.deepEqual(
      await fixture.page.evaluate(() => ({
        installations: globalThis.gwCompanionInstallations,
        runtime: globalThis.gwCompanionRuntime,
        state: globalThis.gwCompanionState,
      })),
      {
...
async function installTargetReadout(page, moduleBytes) {
  return page.evaluate(async (bytes) => {
    ...
    const { installEnhancements } = await import("./enhancements.js");
...
async function assertTargetReadoutLifecycle() {
  ...
  try {
    ...
    assert.deepEqual(
      await fixture.page.evaluate(
        () => globalThis.gwNative.init.enhancementSelection,
      ),
      { nativeCursor: false, targetReadout: true },
    );
    ...
    await fixture.page.waitForFunction(
      () =>
        globalThis.gwCompanionRuntime?.readout?.line
        === "1001 Spellcast",
    );
    assert.equal(
      await fixture.page.locator("#enhancement-target").innerText(),
