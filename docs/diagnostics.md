...
| Every public beta/RC uses exactly latest Stable's durable settings keys and leaves their values, Builds/Teams, window state, and origin-owned browser storage readable and writable by that Stable | release operation | `scripts/verify-stable-beta-roundtrip.ts` launches the exact latest signed Stable → exact newer signed candidate → the same Stable, compares the settings key sets, and proves Stable preserves every untouched candidate value; `.github/workflows/release.yml` makes that proof release-only and mandatory for beta/RC. A production IDBFS/template round-trip is added when Electron, Chromium, or the filesystem/persistence contract changes |
