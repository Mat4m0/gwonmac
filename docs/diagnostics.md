...
| Every public beta/RC leaves canonical data readable and writable by latest Stable | release operation | `scripts/verify-stable-beta-roundtrip.ts` launches latest signed Stable → exact signed candidate → the same Stable; `.github/workflows/release.yml` makes that proof release-only and mandatory for beta/RC |
