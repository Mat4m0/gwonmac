...
| Build and Team authoring remains available on an unknown official client without enabling live observation or Apply | `PRODUCT.md`, `docs/user-guide.md` | `tests/packaged-enhancement-runtime.ts` launches the normal packaged official-only route with Tools selected, persists a Team across renderer reload, proves the unavailable Apply reason, verifies official bytes remain served, and observes no companion runtime or Enhancement resource request |
...
| Stable is default; Beta admits beta/RC but never alpha; an older Stable return never reaches the native updater | Settings, website download, `docs/user-guide.md` | `tests/unit/app-updater.test.ts` and `tests/unit/settings.test.ts` execute the track/refusal/manual-return table; `tests/website-smoke.ts` executes Stable-default and explicit-Beta acquisition |
| Every public beta/RC uses exactly latest Stable's durable settings keys and accepted values, and leaves settings, Builds/Teams, window state, and origin-owned browser storage readable and writable by that Stable | release operation | `scripts/verify-stable-beta-roundtrip.ts` launches the exact latest signed Stable → exact newer signed candidate → the same Stable, compares settings keys, exercises every candidate enum and numeric/null boundary through Stable, and proves Stable preserves every untouched candidate value; `.github/workflows/release.yml` makes that proof release-only and mandatory for beta/RC. A production IDBFS/template round-trip is added when Electron, Chromium, or the filesystem/persistence contract changes |
