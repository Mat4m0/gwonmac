...
| Every public beta/RC leaves settings, Builds/Teams, window state, and origin-owned browser storage readable and writable by latest Stable | release operation | `scripts/verify-stable-beta-roundtrip.ts` launches the exact latest signed Stable → exact newer signed candidate → the same Stable; `.github/workflows/release.yml` makes that proof release-only and mandatory for beta/RC. A production IDBFS/template round-trip is added when Electron, Chromium, or the filesystem/persistence contract changes |
