...
//
// The web port's `Gw.snapshot` is a Guild Wars `.dat` — same container as the
// desktop client, magic and all. `plans/tools/hero-builds/evidence/
// skill-icons-archive.md` records how that was established and every field
// offset below was measured against a real 4.2 GB archive rather than taken
// from a document.
//
// Developer tooling, deliberately. Nothing here runs in the app: this is how a
// skill catalogue gets *extracted* from a client build, not how the app reads
// game data at runtime.
//
// ## No I/O
//
// Every function takes a `ReadAt`. The archive is 4.2 GB spread over 16,023
// content-addressed chunks and cannot be a Buffer, so the caller owns assembly
// and this file owns the format. That also makes the parsing testable against a
// few hundred synthetic bytes instead of a game install.
