...
const SENSITIVE_KEY = /pass|auth|cookie|token|secret|credential|username|email|account/i;
const ABSOLUTE_PATH =
  /(?<=[\s"'(=:])\/(?!\/)[^/\s"',;)}\]]+(?:\/[^/\s"',;)}\]]+)*/g;
...
class Histogram {
  private readonly buckets = Array<number>(DIAGNOSTIC_BUCKETS_US.length).fill(0);
  private total = 0;
  private sum = 0;
  ...
  summary(): DiagnosticHistogramSummary {
    const percentile = (p: number) => {
      ...
      const wanted = Math.ceil(this.total * p);
      let seen = 0;
...
function redactFields(fields: DiagnosticFields | undefined): DiagnosticFields | undefined {
  if (!fields) return undefined;
  return Object.fromEntries(
    Object.entries(fields).flatMap(([key, value]) =>
      SENSITIVE_KEY.test(key)
        ? []
        : [[key, typeof value === "string" ? redactText(value) : value]],
    ),
  );
}
...
class FlightRecorder {
  ...
  readonly startedWall = new Date(Date.now() - process.uptime() * 1_000).toISOString();
  private readonly started =
    process.hrtime.bigint() - BigInt(Math.round(process.uptime() * 1_000_000_000));
  ...
  event(
    ...
    const tsUs = detail.timestampUs ?? this.timestampUs();
    const record: LogRecord = {
      seq: ++this.seq,
      tsUs,
      wallTime: new Date(Date.parse(this.startedWall) + tsUs / 1_000).toISOString(),
      level,
      subsystem,
    ...
    if (detail.parentSpanId !== undefined) record.parentSpanId = detail.parentSpanId;
  ...
  endCapture(
    level: 1 | 2,
    stopReason: CaptureMetadata["stopReason"],
  ): void {
    if (
  ...
  private buildSummary(
    ...
    return {
      ...
      histograms: Object.fromEntries(
        [...histograms].map(([name, histogram]) => [name, histogram.summary()]),
      ),
      latest: { ...latest },
    };
  }
  ...
  async appendFrames(batch: RendererFrameBatch): Promise<void> {
    ...
    this.writes = this.writes.then(async () => {
      ...
      const file = path.join(gamePaths().diagnostics, `frames-${this.sessionId}.bin`);
      if (!this.framesReady) {
      ...
      batch.data.forEach((value, index) => bytes.writeDoubleLE(value, index * 8));
  ...
  private async ensureFile(): Promise<void> {
    if (!this.ready) {
      this.ready = (async () => {
        ...
        this.currentFile = path.join(dir, `session-${this.sessionId}.jsonl`);
        try {
  ...
  private async roll(): Promise<void> {
    const dir = gamePaths().diagnostics;
    const stamped = path.join(dir, `session-${this.sessionId}-${Date.now()}.jsonl`);
    try {
  ...
  private async pruneFiles(keep: number): Promise<void> {
    ...
    await Promise.all(dated.slice(keep).map(({ file }) => rm(file, { force: true })));
  }
}
...
function containsSensitiveText(text: string): boolean {
  const forbidden = [
    ...
    /"authorization"\s*:/i,
    /"cookie"\s*:/i,
    /\b(password|authorization|cookie|token|secret)\b\s*[:=]\s*(?!\[redacted\])[^,\s}"']+/i,
    /([?&][^=\s"'&]+)=(?!\[redacted\])([^&#\s"',}]+)/i,
    new RegExp(ABSOLUTE_PATH.source),
  ];
