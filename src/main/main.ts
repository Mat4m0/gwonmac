...
if (primaryInstance) void app.whenReady().then(async () => {
  ...
  const ipcCleanup = registerIpcHandlers({
    ...
    getTravelHistory: () => preferences.getTravelHistory(),
    recordTravelVisit: (mapId) => preferences.recordTravelVisit(mapId),
    clearTravelHistory: () => preferences.clearTravelHistory(),
