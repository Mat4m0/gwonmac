 ...
 *
 * The bit layout was reconstructed from GWToolbox's party-loadout encoder and
 * checked against independently published codes from ArenaNet's wiki and a
 * separate encoder library. Hand-built vectors in the owning unit test are
 * regression evidence; the external codes are the conformance evidence. The
 * remaining live-only question is the write direction: a hosted client must
 * still prove that it accepts every code this encoder writes.
 *
 ...
 * pasted into Discord. It never returns a partially-filled template: that is
 * exactly the shape a caller misreads as success.
 *
 * Beyond empty, truncated, wrong-kind, wrong-version, oversized-attribute, and
 * non-alphabet inputs, this file refuses four more things. They are forced by
 * the record the result has to fit in: a template that cannot become a `Build`
 * is not a template we can honour.
 *
 ...
 * *skill* ids, which stay bare numbers so a skill
 ...
 * round-trip.
 * 4. Trailing bits that are not zero. The accepted padding variants differ
 * only by trailing zero bits. Accepting non-zero trailing bits would make an
 ...
 * length up to the 84-character format maximum, which keeps both
 * padding rules valid.
 *
 * Application-level rules stay out of here: whether the player's
 ...
 *
 * The party-loadout format carries the same body under an 8-bit hero id instead
 * of the 8-bit `kind`/`version` head. The shared
 ...
 * where a body ends — makes it a second reader of the width rules, and the
...
/** RFC 4648 standard order, read least-significant bit first. */
const ALPHABET =
  "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

/** Field 0. Kind 15 is the party-loadout/equipment family and must not decode here. */
const SKILL_TEMPLATE_KIND = 14;

/** Field 1. The only version observed in ArenaNet-written codes. */
const SKILL_TEMPLATE_VERSION = 0;

/** The two nibbles above; a party member replaces them with a hero id. */
const HEAD_BITS = 8;

/** `SkillID::No_Skill`. An empty slot is this id at full width, not a short stream. */
const NO_SKILL = 0;

/** `SkillTemplate` holds `attribute_ids[12]`; a larger count is malformed. */
const MAX_ATTRIBUTES = 12;

/** The highest rank attribute points can buy and `AttributeRank` admits. */
const MAX_RANK = 12;

/** `skill_code` is 4 bits, so the widest skill field is 23 bits. */
const MAX_SKILL_ID = (1 << 23) - 1;

/**
 * The largest possible payload is 502 bits, which is 84 characters. Nothing
 * longer can be a template, so a megabyte of `A`s is
...
/**
 * The narrowest supported profession width that holds both ids. Ids stop
 ...
 * string. Silently decoding the valid prefix of a corrupted paste is worse
 * than refusing it.
 */
...
/** Bits back to characters, zero-padded to the next character boundary. */
 ...
 * starting at `offset`. Returns `null` for anything it cannot fully decode.
 *
 * Exported because a party loadout embeds this exact bit range under a hero id
 * instead of the head. It is a bit range and not a string, so the boundary
 * between the two files is drawn there: a caller that had to re-measure the
 * body would be a second reader of the width rules, and two readers of one
...
export function decodeTemplateBody(
  ...
  // for a primary: there is no such character, so there is no such build.
  ...
  // Unconditional: the width code is written even when the count is 0.
  ...
  for (let entry = 0; entry < attributeCount; entry++) {
    ...
    // Rank 0 is kept rather than dropped: the decoder reports what the
  ...
  for (const slot of SKILL_SLOTS) {
    ...
    if (id === null) return null;
    // A hole in the middle of the bar is legal and common, so this
...
export function decodeSkillTemplate(code: string): SkillTemplate | null {
  ...
  // header: this keeps both observed padding forms acceptable while still
 ...
 *
 * Widths are minimal, and deliberately not the ones Toolbox's own writer
 * emits: it never writes `attr_code = 1` or a
...
export function encodeTemplateBody(template: SkillTemplate): number[] | null {
  ...
  // Insertion order is the order the decoder read: observed codes are unsorted,
  // so the sequence is the code's and not ours to normalise.
  ...
  for (const [name, rank] of Object.entries(template.attributes)) {
    // Rank 0 is representable and no observed code spends `a + 4` bits on
 ...
 *
 * ArenaNet's own wiki specifies the two padding rules this function ends with,
 * and four real codes confirm them:
 *
