/**
 * Maps observed Guild Wars map IDs to stable display-only identifiers.
 * Derived from GWCA Constants/Maps.h (gwdevhub/GWToolboxpp, 2026-09-03).
 * This catalogue never grants travel authority.
 */
const GUILD_WARS_MAP_IDENTIFIERS: readonly (string | null)[] = Object.freeze([
  "None",
  "Gladiators_Arena",
  "DEV_Test_Arena_1v1",
  "Test_map",
  ...
  "Dragons_Throat_area__What_Waits_in_Shadow",
...
const MAP_NAME_REPLACEMENTS: Readonly<Record<string, string>> = Object.freeze({
  DAlessio: "D'Alessio",
  Droknars: "Droknar's",
  Lions: "Lion's",
  Glints: "Glint's",
  Griffons: "Griffon's",
  Scoundrels: "Scoundrel's",
  Majestys: "Majesty's",
  Lornars: "Lornar's",
  Tascas: "Tasca's",
  Dragons: "Dragon's",
  ...
  Ravens: "Raven's",
  Vloxs: "Vlox's",
  Frostmaws: "Frostmaw's",
  Gadds: "Gadd's",
  Gunnars: "Gunnar's",
  Longeyes: "Longeye's",
  Destructions: "Destruction's",
  Kilroys: "Kilroy's",
  Fronis: "Fronis'",
  Justiciars: "Justiciar's",
  Hierophants: "Hierophant's",
  Ogdens: "Ogden's",
  Treatys: "Treaty's",
});

function displayMapIdentifier(identifier: string): string {
  const withoutTechnicalSuffix = identifier
    .replace(/_(?:outpost|explorable|mission(?:_?\d+)?|cinematic|cutscene)$/, "")
    .replace(/_pre_[Ss]earing$/, " (pre-Searing)")
    .replace(/_Level_?(\d+)$/, " (Level $1)")
    .replaceAll("__", " — ")
    .replaceAll("_", " ");
  return withoutTechnicalSuffix.split(" ").map((word) =>
    MAP_NAME_REPLACEMENTS[word] ?? word
  ).join(" ");
}

/** Returns a descriptive label only. The result must never authorize travel. */
export function guildWarsMapName(mapId: number): string {
  const identifier = Number.isInteger(mapId) && mapId >= 0
    ? GUILD_WARS_MAP_IDENTIFIERS[mapId]
    : null;
  return identifier === null || identifier === undefined
    || identifier === "None" || identifier.startsWith("Unknown_")
    ? `Unknown map (ID ${mapId})`
    : displayMapIdentifier(identifier);
}
